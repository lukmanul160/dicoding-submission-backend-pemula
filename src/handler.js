...
const getAllBooksHandler = (req, h) => {
  try {
    ...
    if (nameQuery) {
      books = booksData.filter((b) =>
        b.name.toLowerCase().includes(nameQuery.toLowerCase())
      );
    } else if (readingQuery) {
      books = booksData.filter(
        (b) => b.reading === Boolean(parseInt(readingQuery, 10))
      );
    } else if (finishedQuery) {
      books = booksData.filter(
        (b) => b.finished === Boolean(parseInt(finishedQuery, 10))
      );
    } else {
      books = booksData;
    }
